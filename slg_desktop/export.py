...
class Export:
    ...
    def email(self):
        # Get date and csv/json from main window
        for_date = '2023-09-01'
        ...
        with smtplib.SMTP_SSL(email_config['host'], email_config['port'], context=context) as server:
            ...
            msg = EmailMessage()
            msg.set_content(email_config['message'].format(date=for_date, filetype=self.file_type))
            ...
            with open('data/test.csv', 'rb') as fp:
                file_data = fp.read()
            msg.add_attachment(file_data, maintype='text', subtype='csv', filename=self.file_name)
